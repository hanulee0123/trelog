...
- デプロイは GitHub へのプッシュをトリガーに Vercel がビルド・反映します。ブランチ戦略とプレビュー環境の扱いは下記の質問事項で合意を取ってください。

### GitHub リモート設定とプッシュ手順
- まだリモートが未設定のため、GitHub リポジトリ URL を確認して `git remote add origin <repo-url>` を実行してください。
- 初回プッシュ時は現在の作業ブランチを GitHub に登録するため、`git push -u origin <branch-name>` を利用します。
- 以降は通常の `git push` で更新でき、Vercel の自動デプロイが走ります。プッシュ後は Vercel ダッシュボードでビルド結果を確認してください。
